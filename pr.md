...
3. **Common Interface Layer**: Core modules map their SoC-specific pins to a
   ...
   `dts/common/wisblock/` to establish a vendor-neutral bridge, similar to the
   `arduino_uno` header implementation.
4. **Base Board Layer**: Carrier boards (e.g., `rakwireless_rak19007`) are
   implemented as shields. They define physical slot topologies that consume the
   `wisblock_io` interface, remaining completely agnostic to the underlying Core
   SoC.

### Console Routing & Twister Compatibility

Core module definitions intentionally omit the `zephyr,console` chosen node and
the `serial` hardware feature, as core modules have no physical I/O port of
their own. Console routing is delegated to the Base board shield: the shield's
