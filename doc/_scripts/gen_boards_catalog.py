...
def to_sphinx_source_path(path: Path | None):
    if path is None:
        return None

    if path.is_relative_to(ZEPHYR_BASE):
        return path.relative_to(ZEPHYR_BASE).as_posix()

    if path.is_relative_to(PROJECT_ROOT):
        rel = path.relative_to(PROJECT_ROOT)

        if len(rel.parts) >= 5 and rel.parts[0] == 'boards' and rel.parts[1] == 'rakwireless' and rel.parts[3] == 'doc':
            suffix = rel.parts[4:]
            return Path('_generated') / 'boards' / 'rakwireless' / rel.parts[2] / Path(*suffix)

        if len(rel.parts) >= 5 and rel.parts[0] == 'boards' and rel.parts[1] == 'shields' and rel.parts[3] == 'doc':
            suffix = rel.parts[4:]
            return Path('_generated') / 'shields' / rel.parts[2] / Path(*suffix)
...
def guess_image(board_or_shield):
    ...
    source_path = to_sphinx_source_path(img_file)
    if source_path is None:
        return None

    return source_path.as_posix() if isinstance(source_path, Path) else source_path
...
def get_catalog(generate_hw_features=False, hw_features_vendor_filter=None):
    ...
    for board in boards.values():
        ...
        doc_page_path_obj = to_sphinx_source_path(doc_page)
        if doc_page_path_obj is not None:
            doc_page_path = doc_page_path_obj.as_posix() if isinstance(doc_page_path_obj, Path) else doc_page_path_obj
        else:
            doc_page_path = None
